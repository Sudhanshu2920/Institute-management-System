...
public class UpdateAttendence extends javax.swing.JFrame {
     ...
     */public UpdateAttendence(String facultyId){
         this();
...
//Remove rows one by one from the end of the table
           for (int i = rowCount - 1; i >= 0; i--) {
            ...
            String sql="select stu_id from takes where course_id=\""+courseId.getSelectedItem().toString()+"\" and year="+Integer.toString(d.getYear()+1900);
            pst=conn.prepareStatement(sql);
            rs=pst.executeQuery(sql);

            while(rs.next())
            {
                dtm.addRow(new String[]{rs.getString(1)});
            }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Update Attendence");
 ...
 DefaultTableModel dtm = (DefaultTableModel) tableAttendence.getModel();
        int rowCount = dtm.getRowCount();
         try{
                String sql="insert into attendence(course_id,sec_id,semester,year,date,status,stu_id) values(?,?,?,?,?,?,?)";
                pst=conn.prepareStatement(sql);
                for(int i=0;i<rowCount;i++)
                   {
                       pst.setString(1,courseId.getSelectedItem().toString());
                       pst.setString(2, section.getSelectedItem().toString());
                       pst.setString(3,semester);
                       pst.setString(4,Integer.toString(d.getYear()+1900));
                       pst.setString(5,date.getText());
                       pst.setString(6,dtm.getValueAt(i,1).toString());
                       pst.setString(7,dtm.getValueAt(i, 0).toString());
                       pst.executeUpdate();
                    }
